...
async function countries(){
  ...
  export default async function page() {
      ...
      const northCard = northAmerica.map(country => (
        <aside key={country.ccn3} className='gap-4'>
          ...
          <h2 className='text-[15px] w-[35px] text-center'>{country.name.common}</h2>
      ...
      const countryTotal = northAmerica.length
  return (
    <div>
        <h1>Total Number of Countries: {countryTotal}</h1>


        <div className='grid sm:grid-cols-3 md:grid-cols-4 grid-cols-2 gap-y-9 py-[2em] ml-[40px]'>{northCard}</div>
    </div>
  )
}
